...
impl Image {
    pub(crate) fn available_codec(&self) -> &AVCodecContext {
        self.encoder.as_ref().unwrap_or_else(|| {
            self.decoder
                .as_ref()
                .expect("At least one codec must be available")
        })
    }
    ...
    pub(crate) fn try_decoder(&mut self, codec_id: Option<AVCodecID>) -> Result<&AVCodecContext> {
        if self.decoder.is_none() {
            let decoder = AVCodec::new_decoder_with_id(codec_id.unwrap_or_else(|| {
                self.encoder
                    .as_ref()
                    .ok_or(anyhow!("Init decoder need encoder exist."))
                    .unwrap()
                    .id()
            }))?;

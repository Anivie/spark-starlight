...
impl ResizeImage for Image {
    fn resize_to(&mut self, size: (i32, i32)) -> Result<()> {
        let format = self
            ...
            .format
            .as_ref()
            .ok_or(anyhow!("Failed to get format."))?;
        let pixel_format = format.pixel_format(0)?;
        let sws = match &self.utils.sws {
            Some(sws) => sws,
            None => {
                let sws = SwsContext::from_format_context(
                    &self.available_codec(),
                    Some(pixel_format),
                    Some((size.0, size.1)),
                    None,
                )?;
        ...
        self.encoder.as_mut().map(|x| {
            x.set_size(size);
        });

        self.decoder.as_mut().map(|x| {
            x.set_size(size);
        });
    ...
    fn resize_into(&self, size: (i32, i32)) -> Result<Self> {
        ...
        new_image.encoder.as_mut().map(|x| {
            x.set_size(size);
        });

        new_image.decoder.as_mut().map(|x| {
            x.set_size(size);
        });

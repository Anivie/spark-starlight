...
impl AVFilterContext {
    pub(crate) fn new(filter_name: &str, args: &str, graph: &AVFilterGraph) -> Result<Self> {
        ...
        let filter = unsafe { avfilter_get_by_name(filter_name.clone().into_raw()) };

        if filter.is_null() {
            bail!(
                "Could not find filter with name: {}",
    ...
    pub(crate) fn new_with(
        ...
        let filter = unsafe { avfilter_get_by_name(filter.into_raw()) };

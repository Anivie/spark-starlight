...
use anyhow::Result;
use spark_inference::inference::sam::video_inference::{
...
fn main() -> Result<()> {
    ...
    // let mask = sam2.decode_image(SamPrompt::point(675.0,648.0), &encoded)?;
    let mask = sam2.decode_image(
        SamPrompt::both((624.0, 626.0, 125.0, 97.0), (675.0, 648.0)),
        &encoded,
    )?;
